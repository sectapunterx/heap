...
TEST(Routing, MakeIdWithKind) {
  EXPECT_EQ(routingId(QStringLiteral("deadline"), QStringLiteral("LTE-2398")), QString("deadline:LTE-2398"));
}

TEST(Routing, EmptyKindFallsBackToTask) {
  EXPECT_EQ(routingId(QString(), QStringLiteral("LTE-2398")), QString("task:LTE-2398"));
}
...
class MockCenter : public NotificationCenter {
  Q_OBJECT
 public:
  explicit MockCenter(QObject* parent = nullptr) : NotificationCenter(parent) {
  }
  ...
  void simulateActivated(const QString& nid) {
    emit activated(nid);
  }
};
