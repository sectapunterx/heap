...
public:
    ...
    void post(const Notification &n) override {
        ...
        QVariantMap hints;
        if (!n.category.isEmpty()) hints[QStringLiteral("category")] = n.category;
        ...
        const QString iconPath =
            n.iconPath.isEmpty() ? QStringLiteral("dialog-information") : n.iconPath;
        const int expireMs = n.durationSec > 0 ? n.durationSec * 1000 : -1;

        QDBusReply<uint> reply = m_iface.call(
...
std::unique_ptr<NotificationCenter> createLinuxDBus(QObject *parent) {
    if (!QDBusConnection::sessionBus().isConnected()) return nullptr;
